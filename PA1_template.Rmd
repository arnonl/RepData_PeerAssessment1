...
```{r Plots_distribution_of_total_steps, echo=TRUE}
...
```{r Reports_Mean_and_Median_for_total_daily_activity}
...
```{r Shows_Highest_Activity_Time, echo=TRUE}
n <- max(intStepsMean)
indx <- which.max(intStepsMean)
# Prints the hour in the day that shows, on average across all days, the highest activity level
```

At `r strftime(uTimeInterval[indx], format="%H:%M")` (the `r indx`-th interval), on average across all the days in the dataset, we observe the highest activity in terms of the number of steps taken

## Imputing missing values

``` {r Counts_Missing_Valuses}
notNA <-complete.cases(data)
```
The number of rows in the dataset without missing values is: `r sum(notNA)`

To make the dataset complete I advise filling each missing data with the mean number of steps for that 5-minute interval

``` {r Fills_up_missing_values_and_compere_means}
...
```{r Comperative_activity_plot_by_day_type, echo=TRUE}
